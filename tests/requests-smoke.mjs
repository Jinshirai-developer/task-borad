...
const displayed=inbox.items[0];
assert.deepEqual(displayed.actions,["help_offer"]); assert.equal(displayed.authorName,owner.key); assert.equal(displayed.recipientName,member.key); checks+=3;
assert.equal((await owner.json("/api/work-inbox?view=sent")).sentCount,1); checks++;
status(await owner.request("/api/work-inbox?view=invalid"),400,"invalid inbox view");
...
await change(owner,late.id,"savepoint_save",{nextStep:"Another private edit"});
const inlinePath=`/api/teams/${team}/companion/tasks/${late.id}`;
status(await member.request(inlinePath,"POST",{action:"help_offer",entryId:displayed.id,version:displayed.version,expectedUpdatedAt:displayed.updatedAt}),409,"stale displayed request");
const fresh=(await member.json("/api/work-inbox")).items[0];
status(await member.request(inlinePath,"POST",{action:"help_offer",entryId:fresh.id,version:fresh.version,expectedUpdatedAt:fresh.updatedAt}),200,"inline offer");
const helping=await member.json("/api/work-inbox?view=helping");
assert.equal(helping.helpingCount,1); assert.equal(helping.items[0].helperName,member.key); assert.deepEqual(helping.items[0].actions,["help_withdraw"]); checks+=3;
const sent=await owner.json("/api/work-inbox?view=sent"); assert.equal(sent.items[0].status,"helping"); assert.deepEqual(sent.items[0].actions,["help_resolve","help_cancel"]); checks+=2;
assert.equal((await outsider.json("/api/work-inbox?view=helping")).items.length,0); checks++;
assert.equal((await outsider.json("/api/work-inbox?view=sent")).items.length,0); checks++;
...
const handoffItem=(await member.json("/api/work-inbox")).items.find(item=>item.id===handoff.handoff.id);
assert.equal(handoffItem.criteria,"Review done"); assert.deepEqual(handoffItem.actions,["handoff_accept"]); checks+=2;
