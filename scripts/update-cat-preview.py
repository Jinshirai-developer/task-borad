...
Task-specific deployment of task-board:cat-cutout-v13. Never changes account data.
...
PREVIOUS = "task-board-preview-before-cat-v13"
IMAGE = "task-board:cat-cutout-v13"
...
def main():
    ...
    assert config["Config"]["Image"] == "task-board:pet-collection-final"
    ...
    backup = Path(tempfile.mkdtemp(prefix="cat-ui-20260908.", dir=REPO / ".local/backups"))
    ...
    try:
        ...
        stopped = True
        with (backup / "before-cat-ui.dump").open("xb") as output:
        ...
        files = ["index.html", "pet-play.js", "pet-play.css"] + [f"assets/pet/portfolio-cat-{pose}-v3.png" for pose in ("idle", "pet", "hat", "bow")]
        for file in files:
        ...
        if created:
            run("docker", "stop", CURRENT)
            run("docker", "rename", CURRENT, "task-board-preview-cat-v13-failed")
        if renamed:
